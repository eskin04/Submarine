...
public class Book : MonoBehaviour
{
    ...
    public void TweenForward()
    {
        if (mode == FlipMode.RightToLeft)
            TweenTo(ebl, 0.2f, () => { Flip(); });
        else
            TweenTo(ebr, 0.2f, () => { Flip(); });
    }
    ...
    // Coroutine yerine geçen, pürüzsüz çalışan DOTween fonksiyonu
    public void TweenTo(Vector3 to, float duration, System.Action onFinish)
    {
        DOVirtual.Vector3(f, to, duration, (val) =>
        {
            if (mode == FlipMode.RightToLeft)
                UpdateBookRTLToPoint(val);
            else
                UpdateBookLTRToPoint(val);
        })
