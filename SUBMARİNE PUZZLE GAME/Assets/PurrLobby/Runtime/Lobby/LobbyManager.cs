...
namespace PurrLobby
{
    public class LobbyManager : MonoBehaviour
    {
        ...
        public void SetLocalPlayerRole(int roleIndex)
        {
            PlayerRole selectedRole = (PlayerRole)roleIndex;

            SetLocalPlayerRoleInternal(selectedRole);
        }

        public void SetLocalPlayerRoleInternal(PlayerRole role)
        {
            if (!_currentLobby.IsValid) return;

            if (_currentLobby.Members.Exists(x => x.Role == role && x.Id != _currentProvider.GetLocalUserIdAsync().Result))
            {
                PurrLogger.LogWarning("Bu rol zaten dolu!");
                return;
            }

            var localUserId = _currentProvider.GetLocalUserIdAsync().Result;

            if (_currentProvider is Providers.SteamLobbyProvider steamProvider)
            {
                steamProvider.SetPlayerRoleAsync(localUserId, role);
            }
        }
